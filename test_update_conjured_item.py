...
def get_quality_diff(normal_item):
    thing = input = normal_item
    output_item = deepcopy(input)
    update_normal_item(output_item)

    normal_item_diff = normal_item.quality - thing.quality
...
def test_quality_greater_than_0():
    ...
    thing = input = conjured_item
    output_item = deepcopy(input)
    update_normal_item(output_item)

    conjured_item_diff = conjured_item.quality - thing.quality
